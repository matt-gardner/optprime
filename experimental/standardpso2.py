...
class StandardPSO(mrs.MapReduce):
    ...
    def run_batch(self, job, batch, tty):
        ...
        init_data = job.local_data(init_particles, parter=self.mod_partition,
                splits=numtasks)

        loop = job.data_loop({last_swarm: init_data}, self.opts.iters)
        interm_data = job.map_data(loop.last_swarm, self.pso_map,
                splits=numtasks, parter=self.mod_partition)
        loop.last_swarm.close()
        next_swarm = job.reduce_data(interm_data, self.pso_reduce,
                splits=numtasks, parter=self.mod_partition)
        with loop.do_each(output.freq):
            if 'particles' in output.args:
                loop.out_data = next_swarm
            elif 'best' in output.args:
                collapsed_data = job.map_data(next_swarm,
                        self.collapse_map, splits=1)
                loop.out_data = job.reduce_data(collapsed_data,
                        self.findbest_reduce, splits=1)
        loop.last_swarm = next_swarm

        for dataset, iteration in loop.run():
            job.wait(dataset)
            if loop.has_name(dataset, 'last_swarm'):
                if tty:
                    if last_swarm in ready:
                        print >>tty, "Finished iteration", iteration
            particles = []
            if loop.has_name(dataset, 'out_data'):
            ...
            if ((particles or not hasattr(dataset, 'out_data'))
                    and iteration > 0 and not ((iteration - 1) % output.freq)):
                kwds = {}
                if 'iteration' in output.args:
                    kwds['iteration'] = iteration
                if 'particles' in output.args:
